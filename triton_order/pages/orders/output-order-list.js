...
Page({
  ...
  async outputOrderList(){
    ...
    const res = await callCloudFunction('outputOrderList', postData);
    wx.hideLoading()
    if (res.code == 0) {
      if (this.data.page == 1) {
        this.setData({
          outputOrderList: res.data.outputOrderList,
          logisticsMap: res.data.logisticsMap,
          goodsMap: res.data.goodsMap
        })
      } else {
        this.setData({
          outputOrderList: this.data.outputOrderList.concat(res.data.outputOrderList),
