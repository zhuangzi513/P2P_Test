...
Page({
  ...
  async inputOrderList(){
    ...
    const res = await callCloudFunction('inputOrderList', postData);
    wx.hideLoading()
    if (res.code == 0) {
      if (this.data.page == 1) {
        this.setData({
          inputOrderList: res.data.inputOrderList,
          logisticsMap: res.data.logisticsMap,
          goodsMap: res.data.goodsMap
        })
      } else {
        this.setData({
          inputOrderList: this.data.inputOrderList.concat(res.data.inputOrderList),
